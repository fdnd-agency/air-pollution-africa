...
export async function GET({ url, request, params: routeParams }) {
	...
	const city = await resolveCity(routeParams.city)
	if (!city) {
		return new Response(JSON.stringify({ error: `Unknown city "${routeParams.city}".` }), {
			status: 404,
			headers: { 'Content-Type': 'application/json' }
		})
	}
	...
	query.set('filter[city][_eq]', String(city.id))
	...
	// Only this page's points belong to the city — scope measurements to them.
	...
	measurementsQuery.set('filter[sampling_point][_in]', pointIds.join(','))
